...
const zwlr_layer_surface_v1_listener Bar::_layerSurfaceListener = {
    [](void *owner, zwlr_layer_surface_v1 *, uint32_t serial, uint32_t width,
       uint32_t height) {
...
void Bar::show(wl_output *output) {
  ...
  _layerSurface.reset(zwlr_layer_shell_v1_get_layer_surface(
      wlrLayerShell, _surface.get(), output, ZWLR_LAYER_SHELL_V1_LAYER_BOTTOM,
      "net.tapesoftware.Somebar"));
  zwlr_layer_surface_v1_add_listener(_layerSurface.get(),
                                     &_layerSurfaceListener, this);
  auto anchor = topbar ? ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP
...
void Bar::click(Monitor *mon, int x, int, int btn) {
  ...
  if (x > _statusCmp.x) {
    ...
    for (int tag = _tags.size() - 1; tag >= 0; tag--) {
...
void Bar::layerSurfaceConfigure(uint32_t serial, uint32_t width,
                                uint32_t height) {
...
void Bar::render() {
  ...
  auto img = wl_unique_ptr<cairo_surface_t>{cairo_image_surface_create_for_data(
      _bufs->data(), CAIRO_FORMAT_ARGB32, _bufs->width, _bufs->height,
      _bufs->stride)};
...
void Bar::renderTags() {
  for (auto &tag : _tags) {
    ...
    // auto indicators = std::min(tag.numClients,
    // static_cast<int>(_bufs->height/2)); for (auto ind = 0; ind < indicators;
    // ind++) { 	auto w = ind == tag.focusedClient ? 7 : 1;
...
void Bar::setColorScheme(const ColorScheme &scheme, bool invert) {
  _colorScheme = invert ? ColorScheme{scheme.bg, scheme.fg}
                        : ColorScheme{scheme.fg, scheme.bg};
}
static void setColor(cairo_t *painter, const Color &color) {
  cairo_set_source_rgba(painter, color.r / 255.0, color.g / 255.0,
